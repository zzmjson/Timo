...
public class RoleController {
    ...
    @GetMapping("/dateAuth")
    @RequiresPermissions("system:role:auth")
    public String toDateAuth(@RequestParam(value = "ids") Long id, Model model){
        model.addAttribute("id", id);
        return "/system/role/dateAuth";
    }
    ...
    public ResultVo authList(@RequestParam(value = "ids") Role role){
        ...
        authMenus.forEach(x-> System.out.println(x));
        // 获取全部菜单列表
...
//        list.forEach(x-> System.out.println(x));
     ...
     * 获取机构数据权限资源列表
     */
    @GetMapping("/dateAuthList")
    @RequiresPermissions("system:role:auth")
    @ResponseBody
    public ResultVo  dateAuthList(@RequestParam(value = "ids") Role role){
        System.out.println(role);
        // 获取指定角色权限资源
//        Set<Menu> authMenus = role.getMenus();
//        Set<Dept> auths = deptService.findDeptListByRoleId(role.getId());
        Set<Dept> auths = role.getDepts();
        // 获取全部菜单列表
//        List<Menu> list = menuService.getListBySortOk();
        List<Dept> list = deptService.findDeptAll();
        // 融合两项数据
        list.forEach(dept -> {
            if(auths.contains(dept)){
     ...
     * 保存数据授权信息
     */
    @PostMapping("/addDataAuth")
    @RequiresPermissions("system:role:auth")
    @ResponseBody
    @ActionLog(key = RoleAction.ROLE_AUTH, action = RoleAction.class)
    public ResultVo addDataAuth(@RequestParam(value = "id", required = true) Role role,
                                @RequestParam(value = "authId", required = false) HashSet<Dept> depts){
        // 不允许操作管理员角色数据
